...
帧级 MOS 由 ReportGenerator.compute_mos() 统一负责（CLIP-IQA 无参考画质预测），
VLM 无法修改该值；CLIP-IQA 不可用时 overall_mos 为 null，不回退任何默认分。
"""
...
- **MOS 正常区间为 3.5~5.0**：仅当 MOS ≤ 3.5 且检出数量为 0 时，才视为"异常偏低"并考虑补检；MOS 在 3.5~5.0 之间属于正常，不要因 MOS 略低就触发补检
- **MOS 不可用时不基于 MOS 补检**：当 global_mos 显示为 "unavailable"（CLIP-IQA 依赖缺失/权重下载失败/推理异常），不要据此触发补检，仅凭检出数量与证据决策
...
6. **如果 global_mos > 3.5，图像质量在正常范围，不需要补检，直接 accept**（global_mos 为 "unavailable" 时跳过本条，按检出数量决策）
7. 补检（rerun_detector / dispatch_compression / vlm_discover）仅限 global_mos ≤ 3.5 且 detection_count == 0 的场景（global_mos 不可用时不基于 MOS 补检）
